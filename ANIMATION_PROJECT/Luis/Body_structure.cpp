...
void Node::draw_node(){
	...
	glMatrixMode(GL_MODELVIEW);
		glColor3f(1,1,1);
	glm::mat4x4 m2 = get_translate_mat()*get_rotation_mat();
	if(name !="pelvis") //parent 가 있는 경우  				//BVH part 에서 작업됨.
		global_tranform = this->parent->global_tranform*m2;
	else//root 인 경우
	{
		global_tranform = m2; //position
		//std::cout<<m2[3][0]<<" , "<<m2[3][1]<<" , "<<m2[3][2]<<std::endl;
	}
	GLdouble m_r[16];
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
...
Node* Body_structure::get_nodeBylabel(Node* n,int label){
	Node* curr = n;
