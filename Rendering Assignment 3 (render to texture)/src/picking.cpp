...
void Picking::pick_cal(){
	...
	near = Vector3d(temp_near(0),temp_near(1),temp_near(2));
	far = Vector3d(temp_far(0),temp_far(1),temp_far(2));
}
...
void Picking::hit_check(){
	//assume line through near and far point
	//far과 near를 잇는 라인의 vector
	Vector3d v_line = far-near;
	//geometry의 중점과 near의 vector
	...
	if(pick_button){
		for (int i = 0; i < item.size(); i++) {
			...
			Vector3d v_center2near = Vector3d(temp(0),temp(1),temp(2))-near;
