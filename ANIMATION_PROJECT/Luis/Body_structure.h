...
class Node{
public:
	Node(){ num_child = 0;selected =false;}
	virtual ~Node() {}
	void clear_data();
	void set_root();
...
public:
	Body_structure(){ }
	void break_chain(Body_structure &B);
	...
	void update_by_IK(Node* n,Node *target);

	void update_draw_IK(Node* root,Node * target);
	Node find_node(std::string name_n);
	Node* get_nodeBylabel(Node* root,int label);
	...
	std::vector<glm::vec3> root_pos_ori;
	std::vector<glm::vec3*> motion_ZXY_ori;
	...
	clock_t start_time;
	void cliping_frame(int start,int end);
	glm::vec2 edit_point;
private:
