...
void  initialize_render2texture(){
	...
	bool status = checkFramebufferStatus(fbo_id);
	if(!status)
		std::cout << "FBO Fail" << "\n";
...
void draw_rtt(){
	...
	glLoadIdentity();
	gluPerspective(50, float(tex_width)/float(tex_height), 0.1, 10000);
...
void init()
{
	...
	GLfloat materialAmbient[] = {0.3 ,0.3, 0.3, 1};
	GLfloat materialDiffuse[] = {0.3 ,0.3, 0.3, 1};
