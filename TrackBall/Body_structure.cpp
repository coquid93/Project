...
void Node::draw_cone(){
	...
	float angle = glm::acos(glm::dot(z_axis,get_trans_vector())/(z_axis.length()*get_trans_vector().length()));
	...
	glTranslatef(0,0,glm::length(get_trans_vector())/2);

	glScalef(0.2,0.2,glm::length(get_trans_vector())-0.2);
