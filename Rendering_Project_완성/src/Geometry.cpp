...
Geometry::Geometry()
{
	...
	hit = false;
    scale_factor = 1.0f;
    ambient[0] = 0.2; diffuse[0] = 0.8; specular[0] = 1.0;
    ambient[1] = 0.1; diffuse[1] = 0.6; specular[1] = 1.0;
    ambient[2] = 0; diffuse[2] = 0.0; specular[2] = 0.4;
    ambient[3] = 1.0; diffuse[3] = 1.0; specular[3] = 1.0;
...
void Geometry::trans_geo(Vector3d v){
    ...
    glPushMatrix();
    glLoadIdentity();
    glTranslatef( orientation(3,0), orientation(3,1), orientation(3,2));
...
void Geometry::set_light(float r, double x, double y , double z){
        orientation.setIdentity();
        orientation.block<1,4>(3,0) = Vector4d(x,y,z,1.0f);
        glMatrixMode(GL_MODELVIEW);
        glPushMatrix();
...
void Geometry::set_amb_dif_spec(float *amb , float * diff , float * spec){
        for ( int i = 0 ; i < 4 ; i ++){
                ambient[i] = amb[i];
                diffuse[i] = diff[i];
                specular[i] = spec[i];
...
void Geometry::copy_geometry(Geometry *geo){
        std::cout<<"size of vert buf = "<<(geo->vert_Buf.size())<<std::endl;
        for (int i = 0; i < geo->vert_Buf.size(); i++) {
                vert_Buf.push_back(geo->vert_Buf[i]);
        }
        for (int i = 0; i < (geo->norm_Buf.size()); i++) {
                norm_Buf.push_back(geo->norm_Buf[i]);
        }
        for (int i = 0; i < (geo->indices.size()); i++) {
                indices.push_back(geo->indices[i]);
        }
        orientation.setIdentity();
        orientation.block<1,4>(3,0) = geo->orientation.block<1,4>(3,0);
        glMatrixMode(GL_MODELVIEW);
        glPushMatrix();
        glLoadIdentity();
        glTranslatef( orientation(3,0), orientation(3,1), orientation(3,2));
